...
set<Card> opponentCards;
void getOpponentCards() {
	for (int i = 0; i < 53; ++i)
		opponentCards.insert(Card(i));
	for (auto i = myCards.begin(); i != myCards.end(); ++i)
		opponentCards.erase(*i);
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < (int)whatTheyPlayed[i].size(); ++j)
			for (int k = 0; k < (int)whatTheyPlayed[i][j].size(); ++k)
				opponentCards.erase(whatTheyPlayed[i][j][k]);
	return;
}

vector<Card> bestcard;
int whetherAlly = 0;//上个出了牌的是：2 自己 1友方 0敌方
int lastPlayerPosition = 0;//上个出了牌的人的编号
short currentCards[MAX_LEVEL];
int currentCardsCnt;
...
inline void initialize() {
	memset(currentCards, 0, sizeof(currentCards));
	for (auto c : myCards)
		currentCards[card2level(c)]++;
	currentCardsCnt = myCards.size();
	upper = lower = 1e9;
	maxValue = 1e10;
	timesCnt = 0;
	return;
...
int getStraight(int& s, int k, int begin = 0){
	int j, i;
	for (i = begin; i <= 10;){
		while (i <= 10 && currentCards[i] < k)
			++i;
		j = 1;
		while (i + j <= 11 && currentCards[i + j] >= k)
			++j;
...
//判断是否全场最大
bool isTheLargest(){
	return CardCombo(bestcard.begin(), bestcard.end()).findFirstValid(opponentCards.begin(), opponentCards.end()).comboType == CardComboType::PASS;
}

#define BOMB_VALUE -1
#define ROCKET_VALUE -1.2
#define STRAIGHT_VALUE 0.5
#define STRAIGHT2_VALUE 0.5
#define QUADRUPLE2_VALUE 0.1
#define QUADRUPLE4_VALUE 0


//DFS
double evaluate(int depth, int kth, double& score){
	if (currentCardsCnt <= 0 || kth == 6){
		score += 0.1 * timesCnt;
		if (maxValue > score){
			maxValue = score;
			upper = timesCnt;
	...
	switch (kth){
		case 0:
			...
			for (int i = 0; i < MAX_LEVEL - 2; ++i){
				if (currentCards[i] == 4){
					double currentBombValue = BOMB_VALUE - landlordBid * 0.01;
					if (currentBombValue <= 0.5) currentBombValue = 0.5;
					score += currentBombValue;
					currentCards[i] = 0;
					currentCardsCnt -= 4;
					timesCnt++;
					landlordBid *= 2;
			...
			if (currentCards[MAX_LEVEL - 2] == 1 && currentCards[MAX_LEVEL - 1] == 1){//王炸
				double currentRocketVaule = ROCKET_VALUE - landlordBid * 0.01;
				if (currentRocketVaule <= 0.6)currentRocketVaule = 0.6;
				score += currentRocketVaule;
				currentCards[MAX_LEVEL - 2] = 0;
				currentCards[MAX_LEVEL - 1] = 0;
				currentCardsCnt -= 2;
				timesCnt++;
				landlordBid *= 2;
				...
				landlordBid /= 2;
				timesCnt--;
				currentCards[MAX_LEVEL - 2] = 1;
				currentCards[MAX_LEVEL - 1] = 1;
				currentCardsCnt += 2;
				score -= currentRocketVaule;
			}
		case 1:{//顺子
			int s = 0;
			int i = getStraight(s, 1);
			if (s >= 5){
		...
		case 3:{//三带系列
			...
			if (s != 0){
				...
				for (int t = 0; t < 12 && (t < i || t >= i + s); ++t){
					if (currentCards[t] == 1 && single < s){
						wing[single] = t;
						single++;
					}
					else if (currentCards[t] == 2 && pair < s){
						wings[pair] = t;
						pair++;
					}
				}
		...
		case 4:{//四带二
			for (int i = 0; i <= 12; ++i){
				if (currentCards[i] == 4){
					...
					int _pair[2], pair = 0;
					for (int t = 0; t < 12; ++t){
						if (currentCards[t] == 1 && single < 2){
							_single[single] = t;
							single++;
						}
						else if (currentCards[t] == 2 && pair < 2){
							_pair[pair] = t;
							pair++;
						}
					}
					//四带二单
					if (single == 2){
						currentCards[_single[0]] = currentCards[_single[1]] = 0;
						currentCardsCnt -= 2;
						score += QUADRUPLE2_VALUE;
						timesCnt++;
						evaluate(depth + 1, 4, score);
						timesCnt--;
						currentCards[_single[0]] = currentCards[_single[1]] = 1;
						currentCardsCnt += 2;
						score -= QUADRUPLE2_VALUE;
					}
					//四带二对
					if (pair == 2){
						currentCards[_pair[0]] = currentCards[_pair[1]] = 0;
						currentCardsCnt -= 4;
						score += QUADRUPLE4_VALUE;
						timesCnt++;
						evaluate(depth + 1, 4, score);
						timesCnt--;
						currentCards[_pair[0]] = currentCards[_pair[1]] = 2;
						currentCardsCnt += 4;
						score -= QUADRUPLE4_VALUE;
					}
				}
		...
		case 5:{//对子、单牌
			...
			for (int i = MAX_LEVEL-8; i < MAX_LEVEL - 6; ++i){//10 J
				if (currentCards[i] != 0){
					score += 0.9;
					++n;
				}
			}
			for (int i = MAX_LEVEL - 6; i < MAX_LEVEL - 3; ++i){//QKA
				if (currentCards[i] != 0){
					score = score;
					++n;
				}
			}
			for (int i = MAX_LEVEL - 3; i < MAX_LEVEL; ++i){//2、王
				if (currentCards[i] != 0){
					score = score - 0.5;
					++n;
				}
			}
			if (currentCards[MAX_LEVEL - 1] != 0){
				score -= 1;
				++n;
			}
			timesCnt += n;
...
CardCombo solve() {
	if (lastValidCombo.comboType == CardComboType::ROCKET)//火箭弃疗
		return CardCombo();

	int whoInHistory[] = { (myPosition - 2 + PLAYER_COUNT) % PLAYER_COUNT, (myPosition - 1 + PLAYER_COUNT) % PLAYER_COUNT };//上上家、上家
	if (whatTheyPlayed[whoInHistory[1]].back().size() != 0)//上家有出牌
		lastPlayerPosition = whoInHistory[1];
	else
		lastPlayerPosition = whoInHistory[0];

	if (lastValidCombo.comboType == CardComboType::PASS)//上个出牌的竟是我自己
		whetherAlly = 2;
	else {
		if (myPosition == landlordPosition)//我是地主与世界为敌
			whetherAlly = 0;
		else {//我是农民
			if (lastPlayerPosition == landlordPosition)//上个出牌的是地主
				whetherAlly = 0;
			else
				whetherAlly = 1;
		}
	...
	bestcard.clear();
	initialize();
	double value = 1e9;//当前最优出法出完后的估值
	double score = 0;
	double cur_score = evaluate(0, 0, score);

	if (whetherAlly != 2) {//需要跟牌
		int i = 0;
		bestcard.clear();

		//炸弹一定能作跟牌，先搜炸弹
		if (lastValidCombo.comboType == CardComboType::BOMB)
			i = lastValidCombo.comboLevel + 1;//炸弹不能比别人小，至少level i

		for (; i < MAX_LEVEL - 2; ++i){//普通炸弹，从i搜到2（显然没有四个小王或大王）
			initialize();
			if (currentCards[i] == 4){
				currentCards[i] = 0;
				currentCardsCnt -= 4;
		...
		if (currentCards[MAX_LEVEL - 2] == 1 && currentCards[MAX_LEVEL - 1] == 1){//有王炸
			initialize();
			currentCards[MAX_LEVEL - 2] = currentCards[MAX_LEVEL - 1] = 0;
			currentCardsCnt -= 2;
			double score = 0;
			double temp = evaluate(0, 0, score);
			currentCards[MAX_LEVEL - 2] = currentCards[MAX_LEVEL - 1] = 1;
			currentCardsCnt += 2;
			if (value > temp){
				bestcard.clear();
				value = temp;
		...
		//顺子
			...
			for (int i = lastValidCombo.comboLevel + 1; i <= MAX_LEVEL - 3 - s; ++i){
				int j = 0;
				while (i + j <= MAX_LEVEL - 3 && currentCards[i + j] >= c){
					j++;
				}
				if (j >= s){
					initialize();
		...
		//三带系列
		else if (lastValidCombo.comboType == CardComboType::TRIPLET || lastValidCombo.comboType == CardComboType::TRIPLET2 || lastValidCombo.comboType == CardComboType::TRIPLET1 ||lastValidCombo.comboType == CardComboType::PLANE || lastValidCombo.comboType == CardComboType::PLANE2 || lastValidCombo.comboType == CardComboType::PLANE1){
			initialize();
			int c, s;
			if (lastValidCombo.comboType == CardComboType::TRIPLET || lastValidCombo.comboType == CardComboType::TRIPLET2 || lastValidCombo.comboType == CardComboType::TRIPLET1){//普通三带，不是飞机
			...
			for (int i = lastValidCombo.comboLevel + 2 - s; i <= MAX_LEVEL - 2 - s;){
				int j = 0;
				while (i + j < MAX_LEVEL - 2 && currentCards[i + j] == 3){
					j++;
				}
				if (j >= s){//找到飞机
					for (int t = 0; t <= j - s; ++t){
						...
						for (int k = 0; k < MAX_LEVEL; ++k){//找带的牌，优先不拆牌
							if (c == 0 || single == s)
								break;
							if (currentCards[k] == c && single < s && (k < i + t || k >= i + s + t)){
								sc[single] = k;
								single++;
								currentCards[single] -= c;
								currentCardsCnt -= c;
							}
						}
						if (c != 0 && single < s){//拆牌
		...
		//四带二
			...
			int c;
			initialize();
			if (lastValidCombo.comboType == CardComboType::QUADRUPLE2){//标记带牌数
				c = 1;
			}
			else
				c = 2;
		...
		//对子
		{
			...
			for (int i = 0; i < MAX_LEVEL; ++i) {
				if (currentCards[i] >= 2) {
					...
					double temp = evaluate(0, 0, score);
					temp += -0.05 + i / 200.0;
					if (currentCardsCnt == 0)//能一下出完，太棒了！
						temp = -1e9;
					currentCards[i] += 2;
					currentCardsCnt += 2;
					if (value > temp) {
	...
	//最终决策出牌
	if (whetherAlly == 2)//自己随便出
		return CardCombo(bestcard.begin(), bestcard.end());
	else if (whetherAlly == 1){//跟队友的牌
		if (value >= cur_score || (cardRemaining[lastPlayerPosition] <= 3&& cardRemaining[landlordPosition] >=5))//出牌不如不出，或者队友只剩3-的牌且地主还有5+的牌
			return CardCombo();
		else
			return CardCombo(bestcard.begin(), bestcard.end());
	}
	else{//跟对手的牌
		if (myPosition == landlordPosition && value - cur_score >= 4 && cardRemaining[lastPlayerPosition] >= 5)//出牌会导致手牌变得很烂，除非对手快要出完了，不然就过
			return CardCombo();
		else if (value - cur_score >= 3 && cardRemaining[lastPlayerPosition] > 5 && (myPosition - landlordPosition + 3) % 3 == 1)
			return CardCombo();
		else if (value - cur_score >= 4 && cardRemaining[lastPlayerPosition] > 5 && (myPosition - landlordPosition + 3) % 3 == 2)
			return CardCombo();
		else
			return CardCombo(bestcard.begin(), bestcard.end());
	}
...
int main()
{
	...
	if (stage == Stage::BIDDING)
	{
		...
		if ((currentCards[MAX_LEVEL - 1]==1) && (currentCards[MAX_LEVEL - 2]==1)) {
			bidEval += 1.5;
		}
		for (int i = MAX_LEVEL - 4; i < MAX_LEVEL; ++i) {
			bidEval += currentCards[i]*0.6;
		}
		if ((currentCards[MAX_LEVEL - 1] == 0) && (currentCards[MAX_LEVEL - 2] == 0)) {//大小王都没有的时候稳一手
			bidValue = 0;
		}
		else if (bidEval > 2.5) {
			bidValue = 3;
			...
			bidValue = rand() % (3 - maxBid) + maxBid + 1;
		}
		else {
			bidValue = 0;
		}
